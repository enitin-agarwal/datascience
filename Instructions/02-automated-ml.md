...
1. 웹 브라우저에서 https://aka.ms/diabetes-data에 접속해 쉼표로 구분된 데이터를 확인합니다. 그런 다음 이 데이터를 **diabetes.csv**라는 로컬 파일로 저장합니다(저장 위치는 상관없음).
...
5. **설명** 탭을 선택하고 **설명 ID**를 선택한 후에 **집계 중요도** 페이지를 봅니다. 이 차트에는 데이터 세트의 각 기능이 레이블 예측에 영향을 주는 범위가 표시됩니다.
